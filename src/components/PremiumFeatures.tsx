...
const PremiumFeatures: React.FC = () => {
  const pricingTiers: PricingTier[] = [{
    ...
    features: ['One free resume analysis monthly', 'Basic formatting feedback', 'Keyword optimization tips', 'Ad-supported experience'],
    type: 'free',
    adSupported: true
  }, {
    name: 'Premium',
    ...
    features: ['Unlimited resume analyses', 'Cover letter analysis', 'Advanced ATS compatibility testing', 'Industry-specific keyword recommendations', 'Downloadable detailed report', 'Section-by-section feedback', 'Ad-free experience'],
    type: 'subscription',
    isPopular: true,
    duration: 'month'
  }, {
    ...
    features: ['All Premium features for 7 days', 'Unlimited resume analyses', 'Cover letter analysis', 'ATS compatibility testing', 'Downloadable reports', 'Ad-free experience', 'No subscription required'],
    type: 'one-time',
    duration: '7 days'
  }];
  return <div className="py-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {pricingTiers.map((tier, index) => <div key={index} className={`relative rounded-xl border ${tier.isPopular ? 'border-brand-200 shadow-lg shadow-brand-100/50' : 'border-border'} overflow-hidden`}>
            {tier.isPopular && <div className="absolute top-0 right-0">
                <div className="bg-brand-600 text-white text-xs font-semibold px-3 py-1 rounded-bl-lg">
                  Most Popular
                </div>
              </div>}
            
            {tier.adSupported && <div className="absolute top-0 left-0">
            ...
            <div className="p-6">
              ...
              <div className="mt-8">
                <Button className={`w-full ${tier.isPopular ? 'bg-brand-600 hover:bg-brand-700' : ''}`} variant={tier.name === 'Free Basic' ? 'outline' : 'default'}>
